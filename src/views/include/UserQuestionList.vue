<template>
  <div class="row">
    <div class="col mb-lg-3">
      ...
      <!--          문의버튼          -->
      <n-button size="large" style="float:right;" @click="openNewQna" v-show="showQuestBtn">
  ...
  <n-tabs
    ...
    <n-tab-pane name="boardList" tab="boardList" style="overflow-x: auto;">
      <div class="mb-lg-3">
        <n-checkbox
            v-show="userInfo.user_id != null"
            size="large"
            label="내 문의만 조회"
            v-model:checked="myQnaChecked"
    ...
    <n-tab-pane name="detail" tab="detail">
      <div class="container bd-example" style="min-height:600px;">
        ...
        <div class="row">
          ...
          <div class="col col-md-4" style="padding-top:1em; text-align: right;">
              <p class="writer-info">
                <i class="fa fa-user" v-show="isAdmin||detailInfo.user_id==userInfo.user_id"></i>
                {{ isAdmin?detailInfo.user_id:(detailInfo.user_id==userInfo.user_id?'내 문의글':'') }}
                &nbsp;&nbsp;
        ...
        <!--          수정/삭제 버튼           -->
        <div class="row" v-show="detailInfo.user_id == userInfo.user_id || !!isAdmin">
          <div class="col-md-12" style="align-items: end;">
            <n-popover trigger="hover">
              <template #trigger>
                <!--          삭제 버튼(v-show 조건 : 글쓴이/관리자)           -->
                <n-button type="error" style="float:right;" v-show="showDeleteBtn" @click="showDeleteModal=true" ghost circle>
                  <template #icon>
            ...
            <n-popover trigger="hover">
              <template #trigger>
                <!--          수정 버튼(v-show 조건 : 글쓴이)           -->
                <n-button style="margin-right: 0.5em; float:right;" v-show="showModifyBtn" @click="modify" ghost circle>
                  <template #icon>
        ...
        <div class="row">
          <!--          첨부파일           -->
          <div class="col col-md-12" v-show="showAttatchment">
            <n-upload
                abstract
      ...
      <div class="container">
        <AdminAnswerForm :detailInfo="detailInfo" :fetchList="fetchList"/>
      </div>
    </n-tab-pane>
  </n-tabs>
</template>
...
import { getQnaList, removeQna } from "@/api/user_qna.js";
import AdminAnswerForm from "@/views/include/AdminAnswerForm.vue";
...
export default defineComponent({
  ...
  setup(){
    ...
    // 버튼 동작 (문의하기)
    const openNewQna = () => { // Open New QnA Form
      router.push('form');
    }
    ...
    const qnaIdColumn = reactive({
      ...
      filter(value, row) {
        return row.user_id==value.toString();
      },
    });
      ...
      },
      {
        ...
        render(obj) {
          return [
            ...
            obj.user_id==userInfo.value.user_id?h(
                NTag,
                {
                  type:'warning',
                  size:'small',
    ...
    // 문의상세 > Show/Hide 첨부파일
    const showAttatchment = ref(false);

    // 문의상세 > 수정/삭제 버튼
    const showModifyBtn = ref(false);
    ...
    // 문의글 수정
    const modify = () =>{
      router.push('form?qna_id='+detailInfo.value.qna_id);
    }
    ...
    return {
      ...
      rowProps: (row) => {
        return {
          ...
          onClick: () => {
            ...
            fileList.value = [];
            // 첨부파일 상세 숨기기
            showAttatchment.value = false;
            // 수정/삭제버튼 숨기기
            showModifyBtn.value = false;
            showDeleteBtn.value = false;
            // 공개글 or 작성자 or 관리자 => 열람 ㅇ
            if(row.secret_yn == "N" || userInfo.value.user_id == row.user_id || isAdmin.value){
              ...
              if(detailInfo.value.user_id == userInfo.value.user_id){
                // 수정 : 작성자
                showModifyBtn.value = true;
              }
              if(detailInfo.value.user_id == userInfo.value.user_id || isAdmin.value){
                // 삭제 : 작성자 or 관리자
                showDeleteBtn.value = true;
              }
            }else{
              // 비공개
              detailInfo.value.title = '비밀글입니다';
              detailInfo.value.content = '비밀글입니다';
              detailInfo.value.qna_id = '';
              detailInfo.value.user_id = row.user_id;
              detailInfo.value.secret_yn = '';
              detailInfo.value.register_dt = row.register_dt;
              detailInfo.value.answer ='';
              detailInfo.value.answer_dt = null;
              showListBtn.value = true;
            }
            // 탭 이동
      ...
      handleCheckedChange(checked){
        myQnaChecked.value = checked;
        if(checked){
          qnaIdColumn.filterOptionValue = userInfo.value.user_id
        }else{
          qnaIdColumn.filterOptionValue = null;
        }
      },
      remove,
      modify,
